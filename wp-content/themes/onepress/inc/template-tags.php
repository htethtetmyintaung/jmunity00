...
if ( ! function_exists( 'onepress_custom_inline_style' ) ) {
	...
	function onepress_custom_inline_style() {
		...
		?>
			...
			$cover_background_image = get_theme_mod('onepress_page_cover_image');
			if ( $cover_background_image ) {
				echo " .page-header.page--cover { background: url('{$cover_background_image}') center no-repeat; background-size: cover; } .page-header { background: url('{$cover_background_image}') center no-repeat; background-size: cover; }; ";
			}
...
if ( ! function_exists( 'onepress_get_section_contact_data' ) ) {
	/**
	 * Get About data
	 *
	 * @return array
	 */
	function onepress_get_section_contact_data() {
		$boxes = get_theme_mod( 'onepress_contact_boxes' );
		if ( is_string( $boxes ) ) {
			$boxes = json_decode( $boxes, true );
		}
		$page_ids = array();
		if ( ! empty( $boxes ) && is_array( $boxes ) ) {
			foreach ( $boxes as $k => $v ) {
				if ( isset( $v['content_page'] ) ) { //Choose page section to insert iframe tag
					$v['content_page'] = absint( $v['content_page'] );
					if ( $v['content_page'] > 0 ) {
						$page_ids[] = wp_parse_args(
							$v,
...
if ( ! function_exists( 'onepress_footer_site_info' ) ) {
	...
	function onepress_footer_site_info() {
		...
		<span class="sep"> &ndash; </span>All Right Reserved.
		<?php
	}
}
