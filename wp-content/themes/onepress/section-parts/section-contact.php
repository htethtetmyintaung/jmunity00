	...
	<?php if ( ! $onepress_contact_disable ) : ?>
		...
		<?php } ?>
			...
			<div class="<?php echo esc_attr( apply_filters( 'onepress_section_container_class', 'container', 'contact' ) ); ?>">
				...
				<?php
					$page_ids = onepress_get_section_contact_data();
				?>
				...
				<?php
					foreach ( $page_ids as $post_id => $settings ) {
						...
						if ( ! $settings['hide_title'] ) {
							if ( $settings['enable_link'] ) {
								echo "<h3 class='contact-contend-heading'><a href='".esc_url( get_permalink( $post ) )."'>".get_the_title( $post )."</a></h3>";
							} else {
								echo "<h3 class='contact-contend-heading'>".get_the_title( $post )."</h3>";
							}
						}
				...
				<div class="row">
					...
					<div class="col-sm-6 wow slideInUp">
						<div class="address-box">
							...
							<h3 class="contact-info-heading"><?php if ( $onepress_contact_address_title != '' ) {
